...
public class MenuServiceIml implements MenuService {
    ...
    @Override
    public Page<MenuItemResponse> getActiveMenus(PaginationRequest pageRequest) {
        ...
        List<MenuItemResponse> shopDtoList = mapToMenuItemDtoList(menuItemList.getContent());

        return new PageImpl<>(shopDtoList, pageable, menuItemList.getTotalElements());
    }

    private List<MenuItemResponse> mapToMenuItemDtoList(List<MenuItem> menuItems) {
        if (menuItems == null) {
